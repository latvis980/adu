...
Guidelines:
- Preserve architectural terminology and proper nouns (architect names, studio names)
...
- The headline may have TWO lines separated by a newline character. Line 1 is the project/architect. Line 2 is the typology/city/country. Translate BOTH lines. On line 2, translate the typology word (e.g., "Culture" → "Cultura" in Spanish, "Культура" in Russian) and translate city/country names to their standard form in the target language (e.g., "Tokyo, Japan" → "Tokio, Japón" in Spanish). Keep the same format with slashes and commas.
- If the headline is only one line, just translate that one line.
...
The JSON must be properly escaped - use \\n for newlines within headline translations, \\" for quotes, \\\\ for backslashes."""
...
IMPORTANT: If the headline has two lines, preserve the newline (\\n) between them in each translation.
