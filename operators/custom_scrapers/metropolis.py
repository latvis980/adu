...
class MetropolisScraper(BaseCustomScraper):
    ...
    async def _find_headline_in_html_with_ai(self, page, headline: str) -> Optional[dict]:
        ...
        html_context = await page.evaluate("""
            () => {
                const containers = document.querySelectorAll('article, .post, [class*="item"], [class*="card"], [class*="project"]');
                const data = [];
        ...
        context = f"Looking for: '{headline}'\n\n" + "\n".join([f"[{i['index']}] {i['link_text']}" for i in html_context[:15]])
        prompt = f"{context}\n\nWhich index? Number or 'NONE':"
