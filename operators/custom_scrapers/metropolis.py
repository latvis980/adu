...
"""
Metropolis Custom Scraper - HTTP Pattern Approach (Simplified)
Scrapes architecture news from Metropolis Magazine (US architecture/design magazine)

Site: https://metropolismag.com/projects/
Strategy: Extract links matching /projects/* pattern from HTML

Pattern Analysis:
- Article URLs: /projects/article-slug/ (e.g., /projects/sawa-circular-apartment-rotterdam/)
- Non-article URLs: /projects/ (section page), /projects/#breadcrumb
...
class MetropolisScraper(BaseCustomScraper):
    """
    HTTP pattern-based custom scraper for Metropolis Magazine.
    Extracts article URLs matching /projects/* pattern from projects page.
    """

    source_id = "metropolis"
    source_name = "Metropolis Magazine"
    ...
    # Must have content after /projects/ and end with /
    ARTICLE_PATTERN = re.compile(r'^/projects/[a-z0-9-]+/$', re.IGNORECASE)

    # URL patterns to exclude (not articles)
    EXCLUDED_PATTERNS = [
        '/projects/$',      # Section page itself
        '/projects/#',      # Breadcrumb links
        '#breadcrumb',
        '/page/',
        '/author/',
        '/tag/',
    ...
    def _is_valid_article_url(self, path: str) -> bool:
        ...
        Valid articles match: /projects/article-slug/
    ...
    def _extract_articles_from_html(self, html: str) -> List[Tuple[str, str]]:
        ...
        for link in all_links:
            ...
            if href.startswith('/'):
                path = href
            elif href.startswith('https://metropolismag.com'):
            ...
            # Build full URL
            full_url = f"https://metropolismag.com{path}"
    ...
    async def fetch_articles(self, hours: int = 24) -> list[dict]:
        """
        Fetch new articles from Metropolis Magazine.

        Workflow:
        1. Load projects page
        2. Extract all article links matching /projects/* (with deduplication)
...
async def test_metropolis_scraper():
    ...
    print("=" * 60)
    print("Testing Metropolis Magazine HTTP Pattern Scraper")
    print("=" * 60)
